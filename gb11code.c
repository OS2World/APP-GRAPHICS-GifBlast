...
gb11_alloc_coder()
{
	...
	if ((gb11=basic_alloc(sizeof(GB11_CODER))) == NULL)
		return NULL;
	gb11->ac_order_1_cons =
		basic_alloc(ARCON_NCODES*sizeof(ARCON_SMALL_CONTEXT));
	if (gb11->ac_order_1_cons == NULL) {
		basic_free(gb11);
		return NULL;
	}
	return gb11;
...
void
gb11_free_coder(gb11)
GB11_CODER *gb11;
{
	if (gb11 != NULL) {
		if (gb11->ac_order_1_cons != NULL)
			basic_free(gb11->ac_order_1_cons);
		basic_free(gb11);
	}
}
...
GB11_CODER *gb11;
{
	...
	for (i=0; i<ARCON_NCODES; i++)
		arcon_small_init(&(gb11->ac_order_1_cons[i]),ARCON_V11_SMALL_RBSIZE);
